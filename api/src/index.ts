...
import { initializeDatabase, getAsync, allAsync, runAsync } from './database.js';
import { calculateDistance, estimateToll } from './googleMaps.js';
import { generateBudgetPDF } from './pdf.js';
import { hashPassword, comparePassword, generateToken, verifyToken, extractTokenFromHeader } from './auth.js';
import { CalculateRequest, Budget } from './types.js';
...
const start = async () => {
  try {
    ...
    const adminExists = await getAsync("SELECT * FROM users WHERE email = 'admin@cbs.com'");
    if (!adminExists) {
      const hashedPassword = await hashPassword('admin123');
      await runAsync(
        'INSERT INTO users (email, password, name, isAdmin) VALUES (?, ?, ?, 1)',
        ['admin@cbs.com', hashedPassword, 'Administrador CBS']
      );
      console.log('✅ Usuário admin criado: admin@cbs.com / admin123');
    }
